...
Camera::Camera(GLFWwindow *window, glm::vec3 center, glm::vec3 up, float radius,
               float minRadius, float azimuthAngle, float polarAngle) {
  this->center = center;
  this->worldUp = up;
  ...
  this->projection = glm::perspective(
      glm::radians(65.0f), (float)width / (float)height, 0.1f, 700.0f);
...
void Camera::moveHorizontal(float distance) {
  ...
  const glm::vec3 strafeVector =
      glm::normalize(glm::cross(viewVector, worldUp));
